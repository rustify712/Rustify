...
unsafe extern "C" fn host_is_little_endian() -> libc::c_int {
    ...
    let mut bytes: *mut libc::c_char = &mut test as *mut libc::c_int
        as *mut libc::c_char;
...
unsafe extern "C" fn check_search_paths(
    ...
    let mut exit_code: libc::c_int = 0 as libc::c_int;
    let mut path: [libc::c_char; 4096] = [0; 4096];
    let mut path_end: *mut libc::c_char = path
        .as_mut_ptr()
    ...
    while *((*st).arr).offset(offset as isize) as libc::c_int != '\0' as i32 {
        ...
        let mut i: size_t = 0 as libc::c_int as size_t;
        while i < *needed_not_found {
            let mut soname_len: size_t = strlen(
                ((*st).arr)
                    .offset(*((*needed_buf_offsets).p).offset(i as isize) as isize),
            );
            if search_path_end
...
unsafe extern "C" fn interpolate_variables(
    ...
    loop {
        let mut dollar: *mut libc::c_char = strchr(
            ((*st).arr).offset(curr_src as isize),
            '$' as i32,
        );
        if dollar.is_null() {
            break;
...
unsafe extern "C" fn print_error(
    ...
    while i_0 < depth {
        if (*s).found_all_needed[i_0 as usize] != 0 {
            let mut len: libc::c_int = (::core::mem::size_of::<[libc::c_char; 5]>()
                as libc::c_ulong)
            ...
            let mut len_0: libc::c_int = (::core::mem::size_of::<[libc::c_char; 7]>()
                as libc::c_ulong)
                .wrapping_sub(1 as libc::c_int as libc::c_ulong) as libc::c_int;
            memcpy(
                p as *mut libc::c_void,
...
unsafe extern "C" fn recurse(
    ...
    let mut is_little_endian: libc::c_int = (e_ident[5 as libc::c_int as usize]
        as libc::c_int == '\u{1}' as i32) as libc::c_int;
    ...
    let mut seen_before: libc::c_int = visited_files_contains(
        &mut (*s).visited,
        &mut finfo,
    );
    if seen_before == 0 {
    ...
    if should_recurse == 0 {
        let mut print_name: *mut libc::c_char = if soname
            == 0xffffffffffffffff as libc::c_ulong || (*s).path != 0
        {
            current_file
        } else {
        ...
        let mut highlight: libc::c_int = (seen_before == 0 && in_exclude_list == 0)
            as libc::c_int;
    ...
    if !last_slash.is_null() {
        let mut bytes: size_t = last_slash.offset_from(current_file) as libc::c_long
            as size_t;
        memcpy(
    ...
    fclose(fptr);
    let mut print_name_0: *mut libc::c_char = if soname
        == 0xffffffffffffffff as libc::c_ulong || (*s).path != 0
    {
        current_file
    } else {
    ...
    let mut highlight_0: libc::c_int = (seen_before == 0 && in_exclude_list == 0)
        as libc::c_int;
    print_line(
        depth,
        print_name_0,
...
unsafe extern "C" fn parse_ld_library_path(mut s: *mut libtree_state_t) {
    ...
    let mut search: *mut libc::c_char = ((*s).string_table.arr)
...
unsafe fn main_0(
    ...
    if opt_help != 0 || opt_version == 0 && positional == 0 as libc::c_int {
        ...
        let mut num_excluded: size_t = (::core::mem::size_of::<
